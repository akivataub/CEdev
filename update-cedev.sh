...
git pull origin main --rebase || { echo "$(date): Pull failed" >> "$LOG_FILE"; exit 1; }
git add .
if ! git diff-index --quiet HEAD --; then
...
git push origin main && { echo "$(date): Pushed to GitHub" >> "$LOG_FILE"; } || { echo "$(date): Push failed" >> "$LOG_FILE"; exit 1; }
